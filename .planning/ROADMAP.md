...
- [x] **Phase 4: XGBoost Forecasting + Serving** - Train, validate, cache, and serve the price forecasting baseline (completed 2026-03-03)
...
- [ ] 04-06-PLAN.md — Gap closure: Playwright E2E tests for chart rendering, fallback banner, and cascading select reset (UI-02, UI-05)
...
| 4. XGBoost Forecasting + Serving | 6/6 | Complete   | 2026-03-08 |
| 5. Soil Crop Advisor | 2/2 | Complete    | 2026-03-03 |
| 6. Mandi Arbitrage Dashboard | 2/2 | Complete    | 2026-03-03 |
| 7. ML Production Hardening | 4/4 | Complete   | 2026-03-09 |

### Phase 7: ML Production Hardening

**Goal:** Every forecast shown to a farmer is either trustworthy or explicitly labelled as insufficient data — block corrupted models, fix confidence signals to reflect actual MAPE, fix direction signal to only show UP/DOWN when the uncertainty band does not straddle zero, raise interval correction threshold, and add stale data warnings.
**Depends on:** Phase 4 (forecast serving layer)
**Requirements**: PROD-01, PROD-02, PROD-03, PROD-04, PROD-05
**Success Criteria** (what must be TRUE):
  1. No corrupted model (prophet MAPE > 5.0) ever reaches the serving layer — they are redirected to seasonal fallback with an explicit "Insufficient data" label
  2. Confidence badge (Green/Yellow/Red) is derived from actual out-of-sample prophet MAPE thresholds: Green < 0.15, Yellow 0.15–0.35, Red > 0.35 — not from any hardcoded or arbitrary value
  3. Direction signal (UP/DOWN/FLAT) is suppressed and replaced with "UNCERTAIN" when the forecast uncertainty band spans both positive and negative change
  4. Interval correction in service.py applies when coverage < 0.80 (not 0.70), widening the band proportionally so displayed intervals are honest
  5. Every forecast response includes a data freshness field; the UI shows a visible warning when last_data_date is more than 30 days ago
**Plans:** 4/4 plans complete

Plans:
- [ ] 07-01-PLAN.md — Wave 0 backend test stubs (7 failing tests for PROD-01..05 in test_forecast_service.py)
- [ ] 07-02-PLAN.md — Wave 0 frontend test stubs (3 failing Vitest tests for stale banner, Red chart gate, UNCERTAIN badge)
- [ ] 07-03-PLAN.md — Backend hardening (service.py all 5 PROD fixes + schemas.py 4 new fields)
- [ ] 07-04-PLAN.md — Frontend rendering (TypeScript types + page.tsx stale banner, chart gate, UNCERTAIN badge, farmer footer)
