...
def list_seasonal_states(
    commodity: str | None = Query(None, description="Filter to states that have data for this commodity"),
    db: Session = Depends(get_db),
):
    """Return sorted distinct state names that have seasonal price stats, optionally filtered by commodity."""
    if commodity:
        rows = db.execute(
            text(
                "SELECT DISTINCT state_name FROM seasonal_price_stats"
