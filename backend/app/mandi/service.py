...
# ---------------------------------------------------------------------------
# Module-level TTL cache for near-static lookup data (states / districts).
# Avoids hitting the DB on every request for data that rarely changes.
# ---------------------------------------------------------------------------
_CACHE_TTL_SECONDS = 3600  # 1 hour

_states_cache: dict[str, object] = {"data": None, "expires_at": 0.0}
_districts_cache: dict[str, dict[str, object]] = {}  # keyed by state name


def _get_cached_states() -> list[str] | None:
    if _states_cache["data"] is not None and time.monotonic() < _states_cache["expires_at"]:
        return _states_cache["data"]  # type: ignore[return-value]
    return None


def _set_cached_states(data: list[str]) -> None:
    _states_cache["data"] = data
    _states_cache["expires_at"] = time.monotonic() + _CACHE_TTL_SECONDS


def _get_cached_districts(state: str) -> list[str] | None:
    entry = _districts_cache.get(state)
    if entry is not None and time.monotonic() < entry["expires_at"]:
        return entry["data"]  # type: ignore[return-value]
    return None


def _set_cached_districts(state: str, data: list[str]) -> None:
    _districts_cache[state] = {
        "data": data,
...
class MandiService:
    ...
    def get_batch_top_prices(self, mandi_ids: list[UUID], limit_per_mandi: int = 3) -> dict[UUID, list[dict]]:
        ...
        # Only look at prices from the last 90 days to avoid scanning the entire history
        price_date_cutoff = (datetime.now() - timedelta(days=90)).date()
        ...
        subquery = self.db.query(
            ...
            PriceHistory.price_date >= price_date_cutoff,
        ).group_by(
            PriceHistory.mandi_id,
            PriceHistory.commodity_id
        ).subquery()
    ...
    def get_states(self) -> list[str]:
        """Get all unique states with mandis (cached for 1 hour)."""
        cached = _get_cached_states()
    ...
    def get_districts_by_state(self, state: str) -> list[str]:
        """Get all districts in a specific state (cached for 1 hour)."""
        cached = _get_cached_districts(state)
        if cached is not None:
            return cached
        try:
            ...
            _set_cached_districts(state, result)
            return result
        except Exception as e:
