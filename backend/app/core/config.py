...
class Settings(BaseSettings):
    ...
    data_gov_api_key: Optional[str] = Field(
        default=None,
        description="data.gov.in primary API key for fetching mandi prices",
    )

    data_gov_api_keys_fallback: Optional[str] = Field(
        default=None,
        description="Comma-separated fallback data.gov.in API keys, tried in order when primary fails",
    )

    @property
    def all_data_gov_keys(self) -> list[str]:
        """All API keys in priority order (primary first, then fallbacks)."""
        keys = []
        if self.data_gov_api_key:
            keys.append(self.data_gov_api_key)
        if self.data_gov_api_keys_fallback:
            for k in self.data_gov_api_keys_fallback.split(","):
                k = k.strip()
    ...
    sync_lookback_days: int = Field(
        ...
        ge=1,
        le=30,
        description="Max days to look back for incremental sync (exact date filter per day)",
    )
