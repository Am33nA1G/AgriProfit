...
def test_forecast_endpoint_14day(client):
    ...
    mock_response = _make_forecast_response(horizon_days=7)

    with patch("app.forecast.routes.is_good_7d_slug", return_value=True), \
         patch("app.forecast.routes.ForecastService7D") as MockService7D:
        MockService7D.return_value.get_forecast.return_value = mock_response
        resp = client.get("/api/v1/forecast/Wheat/Pune")
