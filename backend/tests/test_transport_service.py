...
class TestCompareMandis:
    ...
    def test_compare_multiple_mandis(self, sample_request, sample_mandis, mock_db, monkeypatch):
        ...
        # Results must be sorted by verdict tier (excellent first), then by
        # net_profit descending within each tier.
        VERDICT_ORDER = {"excellent": 0, "good": 1, "marginal": 2, "not_viable": 3}
        sort_keys = [(VERDICT_ORDER.get(c.verdict, 9), -c.net_profit) for c in result]
    ...
    def test_verdict_first_ordering_local_excellent_beats_distant_good(
        self, mock_db, monkeypatch
    ):
        """A local mandi with 'excellent' verdict must rank above a distant
        mandi with 'good' verdict, even when the distant mandi has marginally
        higher net profit (the scenario from Thirurangadi vs Sampath Nagar)."""
        from app.transport.price_analytics import PriceAnalytics
        from datetime import date
        ...
        # Local mandi (0 km, lower price but excellent after behavioral correction)
        # Distant mandi (700+ km, higher price but downgraded to 'good')
        mandis = [
            {
                "id": None,
                "name": "Local Mandi",
        ...
        def mock_distance(*args, **kwargs):
            # Local: ~0 km, Far: ~750 km
            # Extract lat2 from the positional args
            # routing_service.get_distance_km(lat1, lon1, lat2, lon2, db)
            lat2 = args[2] if len(args) > 2 else kwargs.get("lat2", 30.9010)
            lon2 = args[3] if len(args) > 3 else kwargs.get("lon2", 75.8573)
            from app.transport.routing import _haversine
            dist = _haversine(30.9010, 75.8573, lat2, lon2) * 1.35
        ...
        result, _ = compare_mandis(request, db=mock_db)
        assert len(result) == 2

        VERDICT_ORDER = {"excellent": 0, "good": 1, "marginal": 2, "not_viable": 3}
        # The result with the better (lower index) verdict must come first
        first_tier = VERDICT_ORDER.get(result[0].verdict, 9)
        second_tier = VERDICT_ORDER.get(result[1].verdict, 9)
        assert first_tier <= second_tier, (
            f"Expected verdict-first ordering: {result[0].verdict} should rank "
            f"at or above {result[1].verdict}"
        )
